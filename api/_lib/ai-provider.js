...
АНТИЗАЦИКЛИВАНИЕ: Подсчитай сколько вопросов ты уже задал на этом этапе (смотри историю). Если 2 или больше — переходи в S:2 немедленно, даже если ответ расплывчатый. Не застревай.`,
...
АНТИЗАЦИКЛИВАНИЕ: Если ты уже задавал вопрос в одном направлении и ребёнок не смог развить — ОБЯЗАТЕЛЬНО смени тип явления. Никогда не задавай один и тот же вопрос дважды. Если ребёнок 3 раза говорит "не знаю" или даёт пустые ответы — перейди в S:3 с тем что есть.
...
Оценка качества решения по методу Гина (R в теге):
- R:1 = поверхностное решение "в лоб" — ребёнок назвал очевидный способ без использования ресурсов задачи.
- R:2 = рабочее решение, но использует много ресурсов или его можно сделать проще/элегантнее.
- R:3 = решение близко к ИКР — минимум ресурсов, использует именно то, что есть в задаче.
ИКР задачи: ${task?.ikr || ''}
