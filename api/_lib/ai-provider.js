...
export async function getClaudeResponse({
  ...
  if (!systemPrompt) {
    ...
    if (isFrustrated) {
      ...
      stageOverride = `\n⚡ СТАДИЯ Р (разведка): Если ребёнок описывает действие или механизм ("нужно X", "можно использовать X чтобы Y", "если X то Y") → это уже решение, СРАЗУ S:3. Если просто назвал объект → S:2. Если ${botTurns} >= 3 → S:3 немедленно.`;
    }
...
ВАЖНО — различай два типа ответов:
- Назвал объект/явление ("вибрация", "пол", "руки") → S:2, задай один вопрос про идею.
- Описал действие или механизм ("нужно прислониться к...", "можно использовать X чтобы Y", "если X то Y") → это уже решение, СРАЗУ S:3. Не задавай больше вопросов.
АНТИЗАЦИКЛИВАНИЕ: ${botTurns} ответов бота. Если ${botTurns} >= 3 → S:3 немедленно.`,
